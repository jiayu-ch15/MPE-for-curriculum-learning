...
class MultiAgentEnv(gym.Env):
    ...
    def init_set(self, now_agent_num):
        for agent in self.agents:
            ...
            agent.state.c = np.zeros(self.world.dim_c)
        for i, landmark in enumerate(self.world.landmarks):
            landmark.state.p_pos = np.random.uniform(-3, +3, self.world.dim_p)
            landmark.state.p_vel = np.zeros(self.world.dim_p)

        # for i, landmark in enumerate(self.world.landmarks):
        #     if i < int(self.n/4):
        #         landmark.state.p_pos = np.random.uniform(-1.2, -0.7, self.world.dim_p)
        #         landmark.state.p_vel = np.zeros(self.world.dim_p)
        #     elif i < int(self.n/2) and i >= int(self.n/4):
        #         landmark.state.p_pos[0] = np.random.uniform(-1.2, -0.7, 1)
        #         landmark.state.p_pos[1] = np.random.uniform(+0.7, +1.2, 1)
        #         landmark.state.p_vel = np.zeros(self.world.dim_p)
        #     elif i < int(3 * self.n/4) and i >= int(self.n/2):
        #         landmark.state.p_pos[0] = np.random.uniform(+0.7, +1.2, 1)
        #         landmark.state.p_pos[1] = np.random.uniform(-1.2, -0.7, 1)
        #         landmark.state.p_vel = np.zeros(self.world.dim_p)
        #     else:
        #         landmark.state.p_pos = np.random.uniform(+0.7, +1.2, self.world.dim_p)
        #         landmark.state.p_vel = np.zeros(self.world.dim_p)
